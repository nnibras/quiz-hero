...
// Handle Timer
//this function takes in a boolean parameter, then it checks for falsy or truthy parameter. If parameter is falsy then the timer starts to tick in by keeping track of each second and updating the html to different color if the value of second/count is greater than 60. If the parameter is truthy then it clears the timer or resets it.
...
//this function takes in two parameters. index is the numeric parameter that represents the index of the question and givenAns is the selected answer from the page. The function checks if the current quiz already exists by searching for its id. If it does not exist, it adds a new object to the answers array that contains the id and givenAns of the selected quiz. Finally, it calls the displayAnswers function to update the UI with the current state of the answers array but if it exists then it replaces the existing object with a new object that contains the updated givenAns.
